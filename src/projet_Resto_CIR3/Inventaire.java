...
public class Inventaire{
	...
	public void affInv() {
		for(Map.Entry<String, Pair<Integer, Integer>> ent : this.inv.entrySet()) {
	...
	public void subInvItem(String name, int amounts) {
		if(this.inv.containsKey(name)) {
			...
			if(tmp<0) {tmp = 0;}
			Pair<Integer, Integer> tmpPair = new Pair<Integer, Integer>(this.inv.get(name).getKey() , tmp);
			this.inv.replace(name, tmpPair);
		}else {
